...
void Application::Render()
{
	...
	m_deviceContext->PSSetConstantBuffers(2, 1, &m_constBufferDirLight.p);
	m_deviceContext->PSSetConstantBuffers(4, 1, &m_constBufferSpotLight.p);
	...
	// Directional light
	ZeroMemory(&mapSubresource, sizeof(mapSubresource));
	m_deviceContext->Map(m_constBufferDirLight, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mapSubresource);
	memcpy(mapSubresource.pData, &m_dirLightToShader, sizeof(LIGHT_TO_VRAM));
	m_deviceContext->Unmap(m_constBufferDirLight, NULL);

	// Spot light
	ZeroMemory(&mapSubresource, sizeof(mapSubresource));
	m_deviceContext->Map(m_constBufferSpotLight, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mapSubresource);
	memcpy(mapSubresource.pData, &m_spotLightToShader, sizeof(LIGHT_TO_VRAM));
	m_deviceContext->Unmap(m_constBufferSpotLight, NULL);
...
void Application::CreateConstBuffers()
{
	...
	constBufferDesc.StructureByteStride = sizeof(float);
	constBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	m_device->CreateBuffer(&constBufferDesc, NULL, &m_constBufferDirLight.p);
	...
	constBufferDesc.StructureByteStride = sizeof(float);
	constBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	m_device->CreateBuffer(&constBufferDesc, NULL, &m_constBufferSpotLight.p);
}
...
void Application::InitializeLights()
{
	...
	m_dirLightToShader.color.a = 1.0f;
	...
	m_spotLightToShader.direction.x = -1.0f;
...
void Application::LightControl()
{
	...
	if (m_spotLightToShader.status)
	{
		if (GetAsyncKeyState(VK_DOWN))
			m_spotLightToShader.transform.m128_f32[2] -= 0.01f;
		else if (GetAsyncKeyState(VK_UP))				
			m_spotLightToShader.transform.m128_f32[2] += 0.01f;
		if (GetAsyncKeyState(VK_LEFT))					
			m_spotLightToShader.transform.m128_f32[0] -= 0.01f;
		else if (GetAsyncKeyState(VK_RIGHT))			
			m_spotLightToShader.transform.m128_f32[0] += 0.01f;
		if (GetAsyncKeyState('Z'))						
		    m_spotLightToShader.transform.m128_f32[1] += 0.01f;
